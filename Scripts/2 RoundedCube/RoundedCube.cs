...
public class RoundedCube : MonoBehaviour
{
    ...
    private void Awake()
    {
        //Make sure roundness doesnt go above half the lowest number
        int lowestValue = 0;
        if (xSize != 0 && ySize != 0 && zSize != 0)
        {
            lowestValue = (xSize < ySize) ? xSize : ySize;
            lowestValue = (lowestValue < zSize) ? lowestValue : zSize;
        }

        if (lowestValue != 0 && roundness > (lowestValue / 2))
        {
            roundness = lowestValue / 2;
        }
    ...
    private int CreateTopFace(int[] triangles, int t, int ring)
    {
        ...
        t = SetQuad(triangles, t, v, v + 1, v + ring - 1, v + 2); //v+2 because we're still on the ring for the last coord

        //Keep track of coords on ring and coords in grid
